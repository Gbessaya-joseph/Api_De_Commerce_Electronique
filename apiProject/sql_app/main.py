...
def login(user: schemas.UserLogin, db: Session = Depends(get_db)):
    ...
    return crud.login_user(db, email=user.email, password=user.password)
...
@app.post("/category/{category_id}/product/", response_model=schemas.Product)
def create_product(product: schemas.ProductCreate, category_id: int, db: Session = Depends(get_db)):
    db_product = crud.get_product_by_name(db, name=product.name)
    if db_product:
        raise HTTPException(status_code=400, detail="Product already registered")
    return crud.create_product(db=db, category_id=category_id, product=product)


@app.get("/products_by_name/{name}", response_model=list[schemas.Product])
def read_products_by_name(name: str, db: Session = Depends(get_db)):
    products = crud.get_product_by_name(db, name=name)
    if products is None:
        raise HTTPException(status_code=404, detail="Products not found")
    return products
...
@app.post("/commande/", response_model=schemas.Commande)
...
@app.put("/commande/{commande_id}", response_model=schemas.Commande)
def update_commande(commande_id: int, commande: schemas.Commande, db: Session = Depends(get_db)):
    db_commande = crud.get_commande(db, commande_id=commande_id)
    if db_commande is None:
        raise HTTPException(status_code=404, detail="Commande not found")
    return crud.update_commande(db=db, commande=commande)
...
def delete_commande(commande_id: int, db: Session = Depends(get_db)):
    ...
    if db_commande is None:
        raise HTTPException(status_code=404, detail="Commande not found")
    return crud.delete_commande(db=db, commande_id=commande_id)


#codepromo
@app.get("/codepromo/{codepromo_id}")
def read_codepromo(codepromo_id: int, db: Session = Depends(get_db)):
    db_codepromo = crud.get_codepromo(db, codepromo_id=codepromo_id)
    if db_codepromo is None:
        raise HTTPException(status_code=404, detail="CodePromo not found")
    return db_codepromo


@app.get("/codepromos/", response_model=list[schemas.CodePromo])
def read_codepromos(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    codepromos = crud.get_codepromos(db, skip=skip, limit=limit)
    if codepromos is None:
        raise HTTPException(status_code=404, detail="CodePromos not found")
    return codepromos


@app.post("/codepromo/", response_model=schemas.CodePromo)
def create_codepromo(codepromo: schemas.CodePromoCreate, db: Session = Depends(get_db)):
    return crud.create_codepromo(db=db, codepromo=codepromo)


@app.put("/codepromo/{codepromo_id}", response_model=schemas.CodePromo)
def update_codepromo(codepromo_id: int, codepromo: schemas.CodePromo, db: Session = Depends(get_db)):
    db_codepromo = crud.get_codepromo(db, codepromo_id=codepromo_id)
    if db_codepromo is None:
        raise HTTPException(status_code=404, detail="CodePromo not found")
    return crud.update_codepromo(db=db, codepromo=codepromo)


@app.delete("/codepromo/{codepromo_id}")
def delete_codepromo(codepromo_id: int, db: Session = Depends(get_db)):
    db_codepromo = crud.get_codepromo(db, codepromo_id=codepromo_id)
    if db_codepromo is None:
        raise HTTPException(status_code=404, detail="CodePromo not found")
    return crud.delete_codepromo(db=db, codepromo_id=codepromo_id)
...
def read_category_products(category_id: int, db: Session = Depends(get_db)):
    ...
    if db_category is None:
        raise HTTPException(status_code=404, detail="Category not found")
    return db_category.products


#Apply CodePromo to Panier
@app.post("/user/{user_id}/{panier_id}/apply_codepromo/{codepromo_id}")
def apply_codepromo(user_id: int, panier_id: int, codepromo_id: int, db: Session = Depends(get_db)):
    db_user = crud.get_user(db, user_id=user_id)
    if db_user is None:
        raise HTTPException(status_code=404, detail="User not found")
    ...
    if db_panier is None:
        raise HTTPException(status_code=404, detail="Panier not found")
    if db_panier.user_id != user_id:
        raise HTTPException(status_code=400, detail="Panier does not belong to user")
    db_codepromo = crud.get_codepromo(db, codepromo_id=codepromo_id)
    if db_codepromo is None:
        raise HTTPException(status_code=404, detail="CodePromo not found")
    return crud.apply_codepromo_to_panier(db=db, user_id=user_id, panier_id=panier_id, codepromo_id=codepromo_id)
