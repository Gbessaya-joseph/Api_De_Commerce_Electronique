...
def create_product(db: Session, category_id: int, product: schemas.ProductCreate):
...
def delete_product(db: Session, product_id: int):
    ...
    db.commit()
    return {"message": "Product deleted"} and db.query(models.Product).filter(models.Product.id == product_id).first()
# CRUD Panier utilisateur

def get_panier_by_user(db: Session, user_id: int):
    user=db.query(models.User).filter(models.User.id == user_id).first()
    if user.is_active == False:
        return {"message": "Connecter vous bien svp!"}
...
def create_panier_for_user(db: Session, user_id: int, panier: schemas.PanierCreate):
    product = db.query(models.Product).filter(models.Product.id == panier.product_id).first()
...
def apply_codepromo_to_panier(db: Session, user_id: int, panier_id: int, codepromo_id: int):
    user = db.query(models.User).filter(models.User.id == user_id).first()
    if not user:
        return False
    if not user.is_active:
        return False
    codepromo = db.query(models.CodePromo).filter(models.CodePromo.id == codepromo_id).first()
    panier = db.query(models.Panier).filter(models.Panier.id == panier_id).first()
    if codepromo:
        if codepromo.code == panier.name:
            panier.total = panier.total - (panier.total * codepromo.discount)
            db.commit()
            db.refresh(panier)
            return panier
...
def pay_panier_for_user(db: Session, panier_id: int):
    panier = db.query(models.Panier).filter(models.Panier.id == panier_id).first()
    panier.payed = True
    db.commit()
    db.refresh(panier)
